...
<script type="text/javascript">
	function openTab(text,url,iconCls) {
		if($("#tabs").tabs("exists",text)){
			...
			var content = "<iframe frameboder=0 scrolling='auto' style='width:100%;height:100%' src='${pageContext.request.contextPath}/page/"+url+"'></iframe>"
			$("#tabs").tabs("add",{
				title:text,
				iconCls:iconCls,
				closable:true,
		...
		$("#fm").form("submit",{
			...
			onSubmit:function(){
				...
				if(newPassword =='${currentUser.password}'){
					$.messager.alert("系统提示","原密码和新密码不能一致！");
					return false;
				}
			...
			success:function(result){
				...
				if(result.success){
					$.messager.alert("系统提示","密码修改成功下次登录生效！");
					//重置属性值
					resetValue();
					//关闭对话框
					$("#dlg").dialog("close");
	...
	function logout(){
		$.messager.confirm("系统提示","你确定要退出系统?",function(r){
			if(r){
	...
	<div id="dlg" class="easyui-dialog" style="width:320px;height:250px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		<form id="fm" method="post">
			<table cellspacing="8px">
				<tr>
					<td>用户名：</td>
					<td><input type="text" id="userName" name="userName" value="${currentUser.userName }" readonly="readonly" ></td>
					
				</tr>
				<tr>
					<td>原密码：</td>
					<td><input type="text" id="oldPassword" name="oldPassword" class="easyui-validatebox"  required="true" >&nbsp;<font color="red">*</font></td>
				</tr>
				<tr>
					<td>新密码：</td>
					<td><input type="text" id="newPassword" name="newPassword" class="easyui-validatebox" required="true">&nbsp;<font color="red">*</font></td>
					
				</tr>
				<tr>
					<td>确认新密码：</td>
					<td><input type="text" id="newPassword2" name="newPassword2" class="easyui-validatebox" required="true">&nbsp;<font color="red">*</font></td>
